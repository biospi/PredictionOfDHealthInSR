...
def get_imp_stat_cat(
    ...
    filename = f"{farmname}_{-1}_box_{X_train.shape[1]}_{steps}_{str(p_value).replace('.','_')}.png"
    filepath = output_dir / filename
    print(filepath)
...
def process(output_dir):
    for daytime_prct in [0.9]:
        sun_data_delmas = find_minmax_sun(
            farmname="delmas",
            weather_data_file="weather_data/src/delmas_weather_raw.json",
            daytime_prct=daytime_prct
        )
        sun_data_cedara = find_minmax_sun(
            farmname="cedara",
            weather_data_file="weather_data/src/cedara_weather_raw.json",
            daytime_prct=daytime_prct
        )
        ...
        for t in ["dwt"]:
            for j in [7]:
                main(
                    output_dir / "cedara" / f"{str(daytime_prct).replace('.', '_')}" / f"{t}_explain_{j}_datasetmrnn7_23__A",
                    Path("datasets/cedara_datasetmrnn7_23"),
                ...
                # main(
                #     output_dir / "cedara" / f"{str(daytime_prct).replace('.', '_')}" / f"{t}_explain_{j}_datasetmrnn7_23__B",
                #     Path("datasets/cedara_datasetmrnn7_23"),
                ...
                main(
                    output_dir / "delmas" / f"{str(daytime_prct).replace('.', '_')}" / f"{t}_explain_{j}_datasetmrnn7_17__A",
                    Path("datasets/delmas_datasetmrnn7_17"),
                    preprocessing_steps=["QN", "ANSCOMBE", "LOG"],
                    p=False,
                    n_activity_days=j,
                    transform=t,
                ...
                # main(
                #     output_dir / "delmas" / f"{str(daytime_prct).replace('.','_')}" / f"{t}_explain_{j}_datasetmrnn7_17__B",
                #     Path("datasets/delmas_datasetmrnn7_17"),
                #     preprocessing_steps=["QN", "ANSCOMBE", "LOG", "STDS"],
                #     p=False,
                #     n_activity_days=j,
