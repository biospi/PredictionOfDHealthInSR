...
parameters = {
    "C": [1.0, 316.22776601683796, 100000.0, 31622776.60168379, 10000000000.0],
    "gamma": [
        1e-100,
        1.9306977288832773e-86,
        3.727593720314923e-72,
        7.196856730011588e-58,
        1.3894954943731474e-43,
        2.682695795279738e-29,
        5.179474679231308e-15,
        1.0,
        193069772888327.72,
        3.7275937203150446e28,
        7.196856730011588e42,
        1.3894954943731246e57,
        2.682695795279826e71,
        5.179474679231308e85,
        1e100,
    ],
}
...
def main(transponders, farm, n_top, n_bottom, data_folder, resolution="7D"):
    ...
    if n_bottom is not None:
        files = [
            data_folder / f"{x}.csv"
            for x in transponders[len(transponders) - n_bottom : len(transponders)]
        ]

    ncol = 1
    nrow = len(files)
    fig, axes = plt.subplots(
        nrow, ncol, constrained_layout=True, sharex=True, figsize=(6.2, 5.0)
    )
    all_transponders = []
    ...
    fig.suptitle(
        f"Top {n_top}/{tot} transponders sorted by entropy (top to bottom) for {farm}"
    )
    if n_bottom is not None:
        fig.suptitle(
            f"Bottom {n_top}/{tot} transponders sorted by entropy (top to bottom) for {farm}"
        )
    fig.tight_layout()
...
def show_sorted_transponders(transponders, farm, data_folder):
    ...
    for i in range(0, df_all.shape[0], stride):
        ...
        fig.update_layout(
            title="Transponders sorted by entropy",
